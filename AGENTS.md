...
- HTTP/1.1 only on every hop in 1.0. No HTTP/2 or HTTP/3. `PRI * HTTP/2.0` is a hard close. 1.1 may enable HTTP/2 on the **inner + origin** hops via `protocols.http2` (default off, Reset-only). Client-facing may be h2c when `protocols.http2.clientCleartext` (default off, Reset-only); flag-off PRI still hard close. **D7 stands.**
...
- 1.1 flags (`acceptSOCKS5`/`acceptSOCKS4`, `originalDestination`, `protocols.http2`, `compat.flowREST`) are bootstrap + Reset only (D51). `acceptSOCKS5`/`acceptSOCKS4` enable SOCKS CONNECT on the proxy listener (NO AUTH; peek replay; `gate.acquire` after the request; `serveInterceptConn` without HTTP 200). Flags off keep 1.0 SOCKS-close. `protocols.http2` feeds Handshake NextProtos from the session snapshot (D46). When the flag is on and the leaf ALPN is `h2`, inner capture runs via `http2x.ServeClient` / `roundTripInnerH2` (D53); inner CONNECT and websocket Upgrade on h2 are RST (D48). Flag-off or inner `http/1.1` `PRI` still `http2_inner`. `protocols.http2.clientCleartext` (default off, Reset-only) enables client-facing h2c on PRI leftover (`http2x.ServeConn` PrefaceTail; do not re-read the 24-byte preface from the raw conn); flag-off PRI still hard close before `gate.acquire`. RFC 9113 CONNECT on that session is one origin TCP per stream (D62): 2xx HEADERS then raw splice or intercept AfterAck (no HTTP/1.1 200); handshake failure is not a DATA tunnel (D20). Orig-dest tagged CONNECT stays 400. `originalDestination.enabled` binds the Linux REDIRECT listener (D30); dest-IP Dial only (D57). Ready uses OrigDestBound||OrigDestOff. h2 inner transcodes onto one HTTP/1.1 origin TCP: mutex covers RoundTrip and full body drain, MaxConnsPerHost 1 (D44). Replay of an h2 flow is HTTP/1.1 origin-form with leading-`:` names stripped. **D7 stands.**
- 1.1 flags (`acceptSOCKS5`/`acceptSOCKS4`, `originalDestination`, `protocols.http2`, `compat.flowREST`) are bootstrap + Reset only (D51). `acceptSOCKS5`/`acceptSOCKS4` enable SOCKS CONNECT on the proxy listener (NO AUTH; peek replay; `gate.acquire` after the request; `serveInterceptConn` without HTTP 200). Flags off keep 1.0 SOCKS-close. `protocols.http2` feeds Handshake NextProtos from the session snapshot (D46). When the flag is on and the leaf ALPN is `h2`, inner capture runs via `http2x.ServeClient` / `roundTripInnerH2` (D53); inner CONNECT without `:protocol` and illegal h2 `Upgrade: websocket` stay RST, **no flow** (D48 remainder). Flag-off or inner `http/1.1` `PRI` still `http2_inner`. `originalDestination.enabled` binds the Linux REDIRECT listener (D30); dest-IP Dial only (D57). Ready uses OrigDestBound||OrigDestOff. h2 inner transcodes onto one HTTP/1.1 origin TCP: mutex covers RoundTrip and full body drain, MaxConnsPerHost 1 (D44). Replay of an h2 flow is HTTP/1.1 origin-form with leading-`:` names stripped unless live `protocols.http2.origin` is on. **D7 stands.**
