...
- HTTP/1.1 only on every hop in 1.0. No HTTP/2 or HTTP/3. `PRI * HTTP/2.0` is a hard close. 1.1 may enable HTTP/2 on the **inner + origin** hops via `protocols.http2` (default off, live `setFeature` / D51'). 1.2 may enable client-facing h2c when `protocols.http2.clientCleartext` (default off, Reset-only); flag-off PRI still hard close. **D7 stands.**
...
- No mitmproxy REST, mitmweb, or Python addon surface in 1.0 (ADR 0007). 1.1 optional compat flow REST is a first-party adapter (ADR 0011), default-off, live `setFeature` / `replaceCompat` (D51'). Do **not** put `/compat` on `catalog()` / native `compileRoutes`. `CompatBindings()` is a side table matched after authenticate/authorize.
- 1.1 hop/accept flags the running proxy already honors (`acceptSOCKS5`/`acceptSOCKS4`, `protocols.http2.enabled`, `protocols.websocket` / `connect` / `absoluteForm`, `compat.flowREST`, `rules.enabled`, `ui.enabled`) are live-applyable via `setFeature` (and `replaceCompat` for the compat subtree) (D51', ADR 0013). `protocols.websocket` / `connect` / `absoluteForm` default **on** (D22 carve). Listener **binds and addresses** (`originalDestination` enabled+address, `listeners.proxy.address`, `listeners.management.address`, management TLS files, `metrics.listen`) stay bootstrap + Reset only. `maxConcurrentStreams` continues to ride `replaceAdmission`. `replaceTLS` remains the intercept verb. `setFeature` of `tls.intercept` or Reset-only IDs is `validation_failed`. Hop gates 403 `forbidden` before request-phase rules and before any origin RoundTrip / Dial. Websocket 403 after admission accept at the start of **both** `serveAbsolute` and `serveOrigDestHTTP` (before `resolveThenGuard` / DNS) and in inner `roundTripInner` (403 without `Connection: close`, `stop=false`). Orig-dest tagged CONNECT stays 400 (D57) regardless of `protocols.connect`. `acceptSOCKS5`/`acceptSOCKS4` enable SOCKS CONNECT on the proxy listener (NO AUTH; peek replay; `gate.acquire` after the request; `serveInterceptConn` without HTTP 200). Flags off keep 1.0 SOCKS-close. `protocols.http2` feeds Handshake NextProtos from the session snapshot (D46). When the flag is on and the leaf ALPN is `h2`, inner capture runs via `http2x.ServeClient` / `roundTripInnerH2` (D53); inner CONNECT without `:protocol` and illegal h2 `Upgrade: websocket` stay RST, **no flow** (D48 remainder). Flag-off or inner `http/1.1` `PRI` still `http2_inner`. `protocols.http2.clientCleartext` (default off, Reset-only) enables client-facing h2c on PRI leftover (`http2x.ServeConn` PrefaceTail; do not re-read the 24-byte preface from the raw conn); flag-off PRI still hard close before `gate.acquire`. RFC 9113 CONNECT on that session is one origin TCP per stream (D62): 2xx HEADERS then raw splice or intercept AfterAck (no HTTP/1.1 200); handshake failure is not a DATA tunnel (D20). `originalDestination.enabled` binds the Linux REDIRECT listener (D30); dest-IP Dial only (D57). Ready uses OrigDestBound||OrigDestOff. h2 inner transcodes onto one HTTP/1.1 origin TCP: mutex covers RoundTrip and full body drain, MaxConnsPerHost 1 (D44). Replay of an h2 flow is HTTP/1.1 origin-form with leading-`:` names stripped unless live `protocols.http2.origin` is on. **D7 stands.**
- 1.2 flags are bootstrap + Reset only (D51' remainder): `listeners.proxy.acceptBind` / `acceptUDPAssociate` / `acceptUserPass`, `protocols.http2.clientCleartext` / `origin` / `extendedConnect` / `capturePush` / `grpcDecode`, `protocols.websocket.inspectFrames`. Empty `spec: {}` materializes all false. 1.1 `acceptSOCKS5` remains CONNECT-only unless `acceptBind` / `acceptUDPAssociate` / `acceptUserPass`. Unspecified BIND DST is rejected. GSSAPI is never selected. Nested inner CONNECT without `:protocol` still RST, no flow. grpc-web stays opaque. Overlay YAML stays flags-off. Catalog is 31 `/v1` rows including `features.get`. **D7 stands.**
- Opt-in HTTP proxy 407 is D76 ([ADR 0017](https://github.com/hilather/go-lab-mitmproxy/blob/main/docs/adr/0017-http-proxy-407.md)): `spec.proxy.httpAuth` default-off; live `replaceHTTPAuth` (8th apply verb). Supersedes D17 remainder **only when `httpAuth.enabled`**. Do **not** use `setFeature` (boolean-only; frozen 11-row `features.get`). Do **not** hang 407 on `replaceRules` or `replaceAdmission`. SOCKS `acceptUserPass` stays Reset-only. Compact `status.features.httpAuth` is additive (K10 reopen). Catalog stays 31. Management stays bearer (D6). Overlay stays `httpAuth` off.
