...
Last reviewed: 2026-08-28 (live apply hop 403 / features.get)
Related ADRs: 0002, 0004, 0009, 0010, 0011, 0012, 0013
...
- PROXY-001: `testdata/proxy/absolute-https.txt`, `connect-no-port.txt`, `connect-two-gets.txt`, `connect-hijack.txt`, `upgrade-websocket.txt`, `upgrade-websocket-disabled.txt`, `upgrade-websocket-frames.txt`, `name-imds.txt`, `name-link-local.txt`, `socks5-connect.txt`, `socks5-imds.txt`, `socks4-off.txt`, `pri-close.txt`, `h2c-pri-leftover.txt`. 1.2 BIND/UDP/user-pass live in `internal/proxy/socks_test.go` (two sockets; not PlayTranscript). Nested inner CONNECT without `:protocol` stays RST **no flow**. grpc-web opaque. Unspecified BIND DST does not Listen.
