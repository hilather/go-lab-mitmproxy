...
Last reviewed: 2026-08-26 (origin-h2 response trailers)
...
| HTTP/2 codec | `http2x` StreamID + pseudos, no Dial idents, `DialTLS == nil`, pool refuses redial, origin `PUSH_PROMISE` capture/RST (D65), origin-h2 response trailers + skip 1xx | `internal/http2x` |
| HTTP/2 transcode | two concurrent h2 streams + h1 origin (no `refuses redial`); response `WaitPaused` with a non-empty body does not block a second stream; origin h2 multiplex; origin-h2 response trailers forwarded (gRPC `grpc-status`); `PUSH_PROMISE` stored not forwarded; `validateReplay` rejects `Pushed`; live and replay strip `:` headers; `h2_trailer_dropped`; HTTPS replay releases origin conn (h1 and live-origin h2) | `internal/proxy` |
...
- PROXY-001: `testdata/proxy/absolute-https.txt`, `connect-no-port.txt`, `connect-two-gets.txt`, `connect-hijack.txt`, `upgrade-websocket.txt`, `upgrade-websocket-frames.txt`, `name-imds.txt`, `name-link-local.txt`, `socks5-connect.txt`, `socks5-imds.txt`, `socks4-off.txt`, `pri-close.txt`, `h2c-pri-leftover.txt`. 1.2 BIND/UDP/user-pass live in `internal/proxy/socks_test.go` (two sockets; not PlayTranscript). Nested inner CONNECT without `:protocol` stays RST **no flow**. grpc-web opaque. Unspecified BIND DST does not Listen.
