...
Last reviewed: 2026-08-23 (D61 h2c leftover transcripts)
...
| Proxy protocol | absolute-form GET/POST, hop-by-hop strip, CONNECT Hijack + two GETs, HTTP/2 preface close, SOCKS peek-close (flags off), SOCKS5/4 CONNECT when flags on (IMDS deny, IPv6 BND `::`, intercept without HTTP 200), SOCKS BIND when `acceptBind` (two-reply success, IMDS/unspecified no Listen, hairpin BND, SOCKS4 BIND, flag-off `05 07`; not PlayTranscript), SOCKS5 UDP ASSOCIATE when `acceptUDPAssociate` (echo, domain pin, IMDS/FRAG/hairpin drop, inbound flood cap, control-close teardown, flag-off `05 07`; two sockets, not PlayTranscript), silent-peer stall (second HTTP before HeaderTimeout), resolve-then-guard (name→IMDS, name→link-local), `https://` 400, CONNECT without port, WebSocket 101, Expect strip, HTTP_PROXY ignored | `internal/proxy` + `internal/proxytest`; transcripts in `testdata/proxy` |
| Proxy protocol | absolute-form GET/POST, hop-by-hop strip, CONNECT Hijack + two GETs, HTTP/2 preface close, SOCKS peek-close (flags off), SOCKS5/4 CONNECT when flags on (IMDS deny, IPv6 BND `::`, intercept without HTTP 200), SOCKS BIND/UDP/user-pass when 1.2 flags on (`socks_test.go`, not PlayTranscript), silent-peer stall (second HTTP before HeaderTimeout), resolve-then-guard (name→IMDS, name→link-local), `https://` 400, CONNECT without port, WebSocket 101 (flag-off copy; flag-on `wsx` frames), h2c PRI leftover, RFC 9113 CONNECT, Expect strip, HTTP_PROXY ignored | `internal/proxy` + `internal/proxytest`; transcripts in `testdata/proxy` |
| Proxy protocol | absolute-form GET/POST, hop-by-hop strip, CONNECT Hijack + two GETs, HTTP/2 preface close, SOCKS peek-close (flags off), SOCKS5/4 CONNECT when flags on (IMDS deny, IPv6 BND `::`, intercept without HTTP 200), 1.2 series SOCKS BIND/UDP/user-pass when those flags on (`socks_test.go` in the implementing PRs, not PlayTranscript), silent-peer stall (second HTTP before HeaderTimeout), resolve-then-guard (name→IMDS, name→link-local), `https://` 400, CONNECT without port, WebSocket 101 (flag-off copy; flag-on `wsx` frames), 1.2 series h2c PRI leftover / RFC 9113 CONNECT, Expect strip, HTTP_PROXY ignored | `internal/proxy` + `internal/proxytest`; transcripts in `testdata/proxy` |
...
| HTTP/2 transcode | two concurrent h2 streams + h1 origin (no `refuses redial`); origin h2 multiplex; `PUSH_PROMISE` stored not forwarded; `validateReplay` rejects `Pushed`; live and replay strip `:` headers; `h2_trailer_dropped` | `internal/proxy` |
...
- PROXY-001: `testdata/proxy/absolute-https.txt`, `connect-no-port.txt`, `connect-two-gets.txt`, `connect-hijack.txt`, `upgrade-websocket.txt`, `name-imds.txt`, `name-link-local.txt`, `socks5-connect.txt`, `socks5-imds.txt`, `socks4-off.txt`, `pri-close.txt`, `h2c-pri-leftover.txt`.
- PROXY-001: `testdata/proxy/absolute-https.txt`, `connect-no-port.txt`, `connect-two-gets.txt`, `connect-hijack.txt`, `upgrade-websocket.txt`, `upgrade-websocket-frames.txt`, `name-imds.txt`, `name-link-local.txt`, `socks5-connect.txt`, `socks5-imds.txt`, `socks4-off.txt`. 1.2 BIND/UDP/user-pass live in `internal/proxy/socks_test.go` (two sockets; not PlayTranscript). Nested inner CONNECT without `:protocol` stays RST **no flow**. grpc-web opaque. Unspecified BIND DST does not Listen.
- PROXY-001: `testdata/proxy/absolute-https.txt`, `connect-no-port.txt`, `connect-two-gets.txt`, `connect-hijack.txt`, `upgrade-websocket.txt`, `upgrade-websocket-frames.txt`, `name-imds.txt`, `name-link-local.txt`, `socks5-connect.txt`, `socks5-imds.txt`, `socks4-off.txt`. 1.2 series BIND/UDP/user-pass tests live in `internal/proxy/socks_test.go` in the implementing PRs (two sockets; not PlayTranscript). Nested inner CONNECT without `:protocol` stays RST **no flow**. grpc-web opaque. Unspecified BIND DST does not Listen.
...
- UI-001: `web/` React/TS + Vite (Node **22.14.0**); Vitest login/list/detail/status/reset; no token in web storage; no exploit/fuzzer/repeater; escaped HTML bodies; `ca.spkiSha256` on status; 1.1 list/detail metadata (protocol badge, stream id, trailers, SOCKS dest, original dest); 1.2 `FlowPage.test.tsx` Frames tab (escaped text/hex, gated on `flow.websocket`), PUSH parent/promised ids, remaining gRPC tab when `flow.grpc` is present (escaped text, no `innerHTML`); `make web-test` / `make web-build`; CI job `web`.
- SWAP-001: `examples/labmitm.yaml` (published binds, `allowLegacyClients: true`, recommended `allowHosts`; **1.2 protocol flags stay off**); `examples/mcpjungle/servers/labmitm.json` + `groups/integration.json` (append `labmitm`); `examples/labinfo/services-labmitm.yaml` (catalog id `labmitm`); `TestLabOverlayExample`.
