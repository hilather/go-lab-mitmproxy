...
Last reviewed: 2026-08-25 (origin h2 POST body)
...
| HTTP/2 transcode | two concurrent h2 streams + h1 origin (no `refuses redial`); response `WaitPaused` with a non-empty body does not block a second stream; origin h2 multiplex; origin h2 POST DATA (no content-length); `PUSH_PROMISE` stored not forwarded; `validateReplay` rejects `Pushed`; live and replay strip `:` headers; `h2_trailer_dropped`; HTTPS replay releases origin conn (h1 and live-origin h2) | `internal/proxy` |
