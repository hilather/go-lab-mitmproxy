...
- Operator Flows chrome **live** / **:443 intercept only** and the CONNECT footer/reason sentence are overlay/default copy (`tls.ports:[443]`). They do not read live `replaceTLS` ports from `GET /v1/status` or `GET /v1/state`. HTTP CONNECT captures store hostname only (no target port on `host`).
