...
Last reviewed: 2026-09-12 (h2 receive-window for unread DATA on RST; v1.6.1 notes pointer)
...
| Listener **addresses**, management TLS files, `metrics.listen` | Reset-only |
...
- **HTTP `Proxy-Authorization` default-off.** Empty `spec: {}` stays an unauthenticated HTTP hop (D17 remainder). Opt-in `spec.proxy.httpAuth` (D76 / [ADR 0017](https://github.com/hilather/go-lab-mitmproxy/blob/main/docs/adr/0017-http-proxy-407.md)) is live `replaceHTTPAuth` on `listeners.proxy` only. Overlay stays flags-off. Orig-dest, inner intercept, Replay, SOCKS, and h2c Extended CONNECT are out. HTTP 407 is **not** a network boundary.
...
- **`action.type: throttle` is not a connection shaper.** Each matching request or response body gets the full configured rate. There is no process-wide or per-TCP shared bucket. WebSocket `101` / CONNECT / SOCKS splice bytes are not paced.
...
- h2 client + h1 origin still serializes streams on one origin TCP (D44). Request trailers are dropped toward origin (stored on the flow; `labmitm_h2_trailer_dropped_total`). Origin h2 multiplexes when `protocols.http2.origin` and the inner leaf negotiated `h2` (D64); one CONNECT = one origin TCP. Origin-h2 request DATA is forwarded even when content-length is omitted, including DATA that arrives after the origin has already returned a complete response. DATA that arrives on an inner stream after RST still `WINDOW_UPDATE`s the connection receive window. Unread DATA already buffered when that stream is aborted is also connection-`WINDOW_UPDATE`d. Origin-h2 response trailers (including gRPC `grpc-status`) are forwarded to the inner client and stored on the flow. HPACK header blocks that exceed one frame still set `END_STREAM` on the opening `HEADERS` (`CONTINUATION` cannot carry it).
...
- HTTP hop `Proxy-Authorization` is default-off (D76). Enabling `httpAuth` on a published bind is still operator risk and is **not** a network boundary. SOCKS user-pass is a separate Reset-only plane (`acceptUserPass`). Publishing `:8888` on a LAN is an operator choice with documented risk.
