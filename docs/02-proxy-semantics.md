...
- Client keep-alive on the inner TLS session is allowed. Each inner request / stream is **one flow**. Request-phase `silent` / `hang` use the same intercept stamp as a completed inner flow (`Intercepted`, `TLS`, absolute `https` URL). They must not store path-only `flowFromReq` rows.
