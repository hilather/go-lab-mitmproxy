...
Flags default off. The D42 accept mux peeks in a per-conn goroutine and SOCKS-closes `0x04`/`0x05` while the flags are false. When `acceptSOCKS5`/`acceptSOCKS4` are true, SOCKS CONNECT is served on `listeners.proxy` (NO AUTH; peek replay; gate after request). Orig-dest bind is implemented (`originalDestination` REDIRECT + SO_ORIGINAL_DST; dest-IP Dial only).
