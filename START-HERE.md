...
`serve` binds the HTTP/1.1 forward proxy. Management REST `/v1` binds when `--management-listen` is an address and bearer auth has ≥1 token. `healthcheck` and `mcp-stdio` remain unimplemented and exit 2. Set `tls.intercept: true` to mint a lab CA and intercept HTTPS on listed ports.
