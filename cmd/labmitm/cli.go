...
func requireConfigFlag(args []string, name string, stderr io.Writer) (path string, err error) {
	...
	if *cfg == "" {
		_, _ = fmt.Fprintf(stderr, "labmitm %s: --config is required\n", name)
...
func validateCmd(args []string, stdout, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "labmitm validate: %v\n", err)
		return 1
	}
...
func canonicalizeCmd(args []string, stdout, stderr io.Writer) int {
	...
	if *path == "" {
		_, _ = fmt.Fprintln(stderr, "labmitm canonicalize: --config is required")
		return 2
	}
	st, err := config.LoadFile(*path)
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "labmitm canonicalize: %v\n", err)
		return 1
	}
	var body []byte
	...
	default:
		_, _ = fmt.Fprintln(stderr, "labmitm canonicalize: --format must be yaml or json")
		return 2
	}
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "labmitm canonicalize: %v\n", err)
