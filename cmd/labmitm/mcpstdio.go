...
func mcpStdioCmd(ctx context.Context, args []string, stdout, stderr io.Writer) int {
	...
	if *path == "" {
		_, _ = fmt.Fprintln(stderr, "labmitm mcp-stdio: --config is required")
		return 2
	}
	if *tokenFile == "" {
		_, _ = fmt.Fprintln(stderr, "labmitm mcp-stdio: --token-file is required")
		return 2
	}
	svc, err := app.Boot(ctx, app.Options{BootstrapPath: *path})
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "labmitm mcp-stdio: load %s: %v\n", *path, err)
		return 1
	}
	defer svc.Close()
	allowLegacy := false
	...
	if snap := svc.Active(); snap != nil && snap.Canonical != nil {
		...
		if vErr != nil {
			_, _ = fmt.Fprintf(stderr, "labmitm mcp-stdio: auth: %v\n", vErr)
			return 1
		}
		verifier = v
		raw, rErr := os.ReadFile(*tokenFile)
		if rErr != nil {
			_, _ = fmt.Fprintf(stderr, "labmitm mcp-stdio: token-file: %v\n", rErr)
			return 1
		}
	...
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "labmitm mcp-stdio: %v\n", err)
		return 1
	}
