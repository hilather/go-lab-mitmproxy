...
// DefaultSessionConfig is TTL 12h, idle 4h, max 64 (docs/08, docs/10).
...
// Create issues a new session and CSRF secret. cookieValue is the opaque
// cookie (≥256 bits). csrf is 32 random bytes as hex.
...
// MaxAge is the cookie Max-Age (absolute TTL) so idle refresh can keep the cookie.
