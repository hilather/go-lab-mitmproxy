...
const realmBearer = `Bearer realm="labmitm"`
...
// Bearer with zero usable tokens is not a 1.0 posture. management-listen=off
// is the operator escape. dev-loopback-unauth may bind without tokens.
func (v *Verifier) RequireListen() error {
	...
	case model.MgmtAuthBearer:
		if len(tokens) == 0 {
			return fmt.Errorf("management.auth.mode bearer requires at least one usable token")
		}
		return nil
	default:
