...
// ByteSize is a binary (IEC) byte count. YAML/JSON must use a unit suffix
// such as 10MiB; bare numbers are rejected.
type ByteSize int64

var sizeFields = map[string]bool{
	"maxInFlightBytes": true,
	"maxBytes":         true,
	"maxBodyBytes":     true,
	"spillThreshold":   true,
	"bodyLimit":        true,
...
func ParseByteSize(s string) (ByteSize, error) {
	...
	if i == 0 || i == len(s) {
		return 0, fmt.Errorf("byte size must use an IEC unit (for example 10MiB)")
	}
...
func coerceByteSizeField(obj map[string]any, key string, child any, path string) (domainerr.FieldViolation, bool) {
	...
	case string:
		if looksLikeBareNumber(n) {
			return domainerr.FieldViolation{
				...
				Message: "byte size must use an IEC unit such as 10MiB, not a bare number",
			}, false
		}
	...
	default:
		return domainerr.FieldViolation{
			Path:    path,
			Code:    violationInvalidValue,
			Message: "byte size must be a string such as 10MiB, not a bare number",
		}, false
	}
}
