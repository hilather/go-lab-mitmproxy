...
// rpcError maps a domain error to a JSON-RPC wire error. Data is the same
// domain payload REST exposes in problem+json extensions.
...
func writeRPC(w http.ResponseWriter, status int, err error) {
	...
	if mapped.Code == capabilities.JSONRPCUnauthenticated {
		w.Header().Set("WWW-Authenticate", `Bearer realm="labmitm"`)
	}
