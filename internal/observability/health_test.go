...
func TestEvaluateReadyRequiresProxyStoreAndCA(t *testing.T) {
	ready := Evaluate(Facts{ProxyBound: true, StoreUp: true, MgmtBound: true, CAReady: true})
	if !ready.Live || !ready.Ready {
		t.Fatalf("healthy probe=%+v", ready)
	}

	down := Evaluate(Facts{ProcessDown: true, ProxyBound: true, StoreUp: true, MgmtBound: true, CAReady: true})
	if down.Live || down.Ready {
		t.Fatalf("process down still live/ready: %+v", down)
	}

	noProxy := Evaluate(Facts{StoreUp: true, MgmtBound: true, CAReady: true})
	if noProxy.Ready {
	...
	noStore := Evaluate(Facts{ProxyBound: true, MgmtBound: true, CAReady: true})
	if noStore.Ready || !hasCode(noStore.Warnings, WarnStoreDown) {
		t.Fatalf("missing store=%+v", noStore)
	}

	mgmtOff := Evaluate(Facts{ProxyBound: true, StoreUp: true, MgmtOff: true, CAReady: true})
	if !mgmtOff.Ready {
		t.Fatalf("explicitly-off management must still be ready: %+v", mgmtOff)
	}

	mgmtMissing := Evaluate(Facts{ProxyBound: true, StoreUp: true, CAReady: true})
	if mgmtMissing.Ready {
		t.Fatal("management neither bound nor off must be unready")
	}

	noCA := Evaluate(Facts{ProxyBound: true, StoreUp: true, MgmtBound: true, CAReady: false})
