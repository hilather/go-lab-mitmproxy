	...
	for _, c := range all {
		...
		for _, r := range c.MCP.Resources {
			if _, ok := idx.byRes[r]; ok {
				// labmitm://flows is shared by list + events (different binding).
				continue
			}
			idx.byRes[r] = c
		}
...
// MustLookup returns the capability with id or panics.
func MustLookup(id ID) Capability {
	c, ok := Lookup(id)
	if !ok {
		panic("capabilities: unknown id " + string(id))
	}
	return c
}

// LookupREST returns the capability bound to method+path
// (for example "GET /v1/state").
...
// LookupResource returns the capability bound to a labmitm:// URI template.
...
// REST-only rows use the capability id; others use each MCP tool once.
