...
func TestKnownEnums(t *testing.T) {
	...
	if !KnownOp(OpReplaceStoreCaps) || !KnownOp(OpSetFeature) || !KnownOp(OpReplaceCompat) || !KnownOp(OpReplaceHTTPAuth) || KnownOp("addZone") || KnownOp("replaceProtocols") || KnownOp("replaceProxyAccept") {
		t.Fatal("op set")
	}
