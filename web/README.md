...
Pages: sign-in, flow list, flow detail (headers / textual or hex body / TLS / download), status (including `ca.spkiSha256` and CA PEM download), scoped audit, gated reset. Live update uses `EventSource` `GET /v1/events/stream` with a 3s `GET /v1/flows` poll fallback.
